...
Shader "Hidden/Amazing Assets/Terrain To Mesh/Terrain Textures Exporter"
{
	Properties 
	{
	...
	float4 fragBasemapPackNormals(v2f_img i) : SV_Target
	{
	...
	float4 fragBasemapMask(v2f_img i) : SV_Target 
	{
		...
		{
			float4 diffuse = UNITY_SAMPLE_TEX2D_SAMPLER (_Map0, _Map0, mapUV);
			float smoothnessValue = lerp(smoothness * diffuse.a, diffuse.a, readSmoothnessFromDiffuseAlpha);
	...
	float4 fragBasemapMaskURP(v2f_img i) : SV_Target 
	{
		...
		else
		{
			...
			mask = float4(lerp(GammaToLinearSpaceExact(GammaToLinearSpaceExact(metallic)), metallic, IsGammaSpace()), occlusion, 0, lerp(smoothness * diffuse.a, diffuse.a, readSmoothnessFromDiffuseAlpha));
	...
	float4 fragBasemapMaskHDRP(v2f_img i) : SV_Target 
	{
		...
		else
		{
			...
			mask = float4(GammaToLinearSpaceExact(metallic), occlusion, 0, lerp(smoothness * diffuse.a, diffuse.a, readSmoothnessFromDiffuseAlpha));
	...
	float4 fragGenericHolesmap(v2f_img i) : SV_Target 
	{
		...
		i.uv = RotateUV(i.uv, rotationDegree);
		float4 holesMap = tex2D(_MainTex, i.uv).xxxx;
	...
	{
		float4 c = tex2D(_MainTex, i.uv);
		float4 holes = UNITY_SAMPLE_TEX2D_SAMPLER(_Map0, _Map0, i.uv);
	...
	float4 fragFillColor(v2f_img i) : SV_Target 
	{
		return _Color;
	}

	float4 fragBlendNormalMaps(v2f_img i) : SV_Target 
	{
		float4 A = tex2D(_MainTex, i.uv);
		float4 B = UNITY_SAMPLE_TEX2D_SAMPLER(_Map0, _Map0, i.uv);


		float3 normalA = UnpackNormal(A);
		float3 normalB = UnpackNormal(B);


		float3 f = normalize(float3(normalA.xy + normalB.xy, normalA.z*normalB.z));
	...
	{				    
		...
		Pass	//2
	    {
			...
			CGPROGRAM
			#pragma vertex vert_img
	    	#pragma fragment fragBasemapPackNormals
			ENDCG
		...
		Pass	//20
	    {
			...
			CGPROGRAM
			#pragma vertex vert_img
	    	#pragma fragment fragBlendNormalMaps
			ENDCG
