    ...
    public partial class MB3_MeshCombinerSingle : MB3_MeshCombiner
    {
        public class MB_MeshCombinerSingle_BoneProcessor : MB_IMeshCombinerSingle_BoneProcessor
        {
            ...
            public bool CollectBonesToAddForDGO(MB_DynamicGameObject dgo, Renderer r, bool noExtraBonesForMeshRenderers)
            {
                ...
                /*
                {
                    var bw = dgo._tmpSMR_CachedBoneWeights[0];
                    Debug.LogError("REMOVE   Collecting Bones: " + dgo.name +
                                    "\n   numBones collected" + dgoBones.Length +
                                    "\n   dgo.isSkinnedMeshWithBones: " + dgo.isSkinnedMeshWithBones +
                                    "\n   dgo.numBindPoses: " + dgoBindPoses.Count +
                                    "\n   idx: " + bw.boneIndex0 + "   bw." + bw.weight0 +
                                    "\n   idx: " + bw.boneIndex1 + "   bw." + bw.weight1 +
                                    "\n   idx: " + bw.boneIndex2 + "   bw." + bw.weight2 +
                                    "\n   idx: " + bw.boneIndex3 + "   bw." + bw.weight3);
                }
                */
            ...
            public void InsertNewBonesIntoBonesArray()
            {
                if (combiner.settings.renderType == MB_RenderType.skinnedMeshRenderer)
                {
                    ...
                    if (combiner.LOG_LEVEL >= MB2_LogLevel.trace) Debug.Log("InsertNewBonesIntoBonesArray  name:  numAdding: " + GetBonesToAdd().Count);
                    int bidx = 0;
            ...
            {
                if (combiner.LOG_LEVEL >= MB2_LogLevel.trace) Debug.Log("AddBonesToNewBonesArrayAndAdjustBWIndexes1 for: " + dgo.name + "  numBones: " + dgo._tmpSMR_srcMeshBoneIdx2masterListBoneIdx.Length);
            ...
            public void ApplySMRdataToMesh(MB3_MeshCombinerSingle combiner, Mesh mesh)
            {
                ...
                if (combiner.LOG_LEVEL >= MB2_LogLevel.trace) Debug.Log("ApplySMRdataToMesh   bindposes.Length: " + combiner.bindPoses.Length + "   boneWeights.Length: " + boneWeights.Length);
                /*
                {
                    string s = "bindPoses:\n";
