...
namespace DigitalOpus.MB.MBEditor
{
    public class MB3_MeshBakerEditorFunctions
    {
        ...
        public static Mesh UnwrapUV2(Mesh mesh, float hardAngle, float packingMargin, out bool meshHasChanged)
        {
            UnwrapParam up = new UnwrapParam();
            UnwrapParam.SetDefaults(out up);
            up.hardAngle = hardAngle;
            up.packMargin = packingMargin;
            bool unwrapSucessful = MBVersionEditor.GenerateSecondaryUVSetForUnwrapping(mesh, up);
            if (unwrapSucessful)
            {
            ...
            else
            {
                ...
                bool secondaryUVGenerationSuccesful = MBVersionEditor.GenerateSecondaryUVSetForUnwrapping(newMesh, up);
                if (secondaryUVGenerationSuccesful)
                {
                    Debug.Log("Converted mesh to 32bit mesh, Succesfully unwrapped mesh");
                    meshHasChanged = true;
                    return newMesh;
                }
                else
                {
                    Debug.LogError("Converted mesh to 32bit mesh, Unsuccessfully unwrapped mesh");
                    meshHasChanged = false;
                    return mesh;
                }
            }
